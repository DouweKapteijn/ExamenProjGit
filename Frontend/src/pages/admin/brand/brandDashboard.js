...
function MerkDashboard() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:3001/api/getMerk/${reqId}?page=${page}&perPage=${perPage}`,
        {
          headers: {
    ...
    <div className="p-4 sm:ml-64">
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right">
          ...
          <tbody>
            {brand.map((brand) => (
              <tr key={brand._id} className="odd:bg-green even:bg-light-green">
                <td className="px-6 py-4">{brand ? brand.brandName : "-"}</td>
                <td className="px-6 py-4">
                  {brand.animal_id ? brand.animal_id.animalName : "-"}
                </td>
                <td className="px-6 py-4">
                  <Link to={`/adminEditBrand/${brand._id}`}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512 512"
                      className="h-8"
                ...
                <td className="px-6 py-4">
                  <Link to={`/adminDeleteBrand/${brand._id}`}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 500 500"
                      className="h-8"
